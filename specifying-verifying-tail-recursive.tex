...
\titlerunning{Verifying tail-recursion in Agda}
...
\lstinputlisting[firstline=32, lastline=49]{code/Instances.agda}
...
The generated tail recursive function has some caveats. Though it avoids mapping over the
entire recursive call result, it introduces inefficiency by doing nested concatenations
to the left, which leads to quadratic complexity. This could be solved by using higher
order functions as the accumulating monoid \cite{hughes1986novel}, but proving the monoid
laws will require us to be able to transform extensional equality to intensional
equality, which is not possible in Agda without using \emph{cubical type theory}
\cite{botta2021extensional,vezzosi2021cubical}, which is out of the scope of this paper.
